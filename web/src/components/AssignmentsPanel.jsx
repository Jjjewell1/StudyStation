...
export default function AssignmentsPanel({ courses, grouped, loading, error }) {
  const [selected, setSelected] = useState('all')

  const visible = selected === 'all' ? grouped : grouped.filter((c) => c.id === selected)
    ...
    <div className="glass flex flex-col rounded-3xl p-5">
      <div className="mb-4 flex items-center justify-between">
        ...
        <span className="text-xs text-white/45">
          {loading ? '…' : `${visibleCount}${selected === 'all' ? '' : ` / ${totalCount}`}`}
        </span>
      </div>

      {!loading && <CourseTabs courses={courses} activeId={selected} onChange={setSelected} />}
